...
export default function DashboardProdutos() {
  ...
  const { produtos, loading } = useSelector(
  ...
  const onSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (editandoId != null) {
      dispatch(updateProduto(form as any) as any);
    } else {
      dispatch(addProduto(form as any) as any);
    }
    setEditandoId(null);
  };

  const handleDelete = (id: number) => {
    dispatch(deleteProduto(id) as any);
  };
  ...
  return (
    <div className="p-6 space-y-6 flex flex-col justify-center items-center">
      ...
      <form
        onSubmit={onSubmit}
        className="space-y-4 p-4 bg-white rounded shadow w-80"
      >
        <div>
